...
- **Web UI**: a React interface with search-as-you-type, a readable article
  preview (with a table of contents, category tags, and every word of the
  running text clickable through to its own Wikipedia page), a
  random-article button, and shareable/deep-linkable URLs (including
...
Beyond the wikitext's own `[[...]]` links, every remaining word of the
rendered text is *also* made clickable, linking to that word's own page
(`Wolf` → `/api/wiki/page/Wolf`, etc.) - a lightweight "hyperdictionary" feel
similar to Wiktionary's inter-linking. This runs as a final pass over the
already-rendered HTML, walking only the plain-text stretches between
existing tags (so it never touches text already inside an `<a>`, and never
touches HTML attributes) and wrapping each word-like token in a link. Most
individual words won't have their own article, so many of these links 404
when clicked - by design; this is a "click anything" affordance, not a
